...
test("integrity: install fails when registry tarball is corrupted", async () => {
  await withTempDir(async (dir) => {
    ...
    // Corrupt the published tarball (index.json still contains the original integrity).
    const publishedTarball = path.join(registryRoot, "@s", "dep-skill", "-", "dep-skill-1.0.0.tgz");
    const buf = await fs.readFile(publishedTarball);
    buf[0] = (buf[0] + 1) % 256;
    await fs.writeFile(publishedTarball, buf);
